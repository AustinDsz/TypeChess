...
@Component({
  ...
  template: `
    <div class="flex flex-col gap-3 w-full">
      ...
      <div
        [class]="chessEngine.turn() === 'b' ? 'bg-slate-900 border-amber-500/50 ring-2 ring-amber-500/20' : 'bg-slate-900/60 border-slate-800'"
        class="flex items-center justify-between p-3 rounded-2xl border transition-all duration-200"
      >
        <div class="flex items-center gap-3">
          <div class="w-9 h-9 rounded-xl bg-slate-950 border border-slate-700 flex items-center justify-center shadow-inner">
            <div class="w-4 h-4 rounded-full bg-slate-900 border-2 border-slate-400"></div>
          </div>
          <div>
            <div class="flex items-center gap-2">
              <span class="font-semibold text-sm text-slate-200">Black Player</span>
            ...
            <div class="flex items-center gap-0.5 mt-0.5 min-h-[20px]">
              ...
              <span
                *ngIf="chessEngine.capturedPieces().materialAdvantage.color === 'b'"
                class="text-[11px] font-bold text-amber-400 ml-2"
              >
        ...
        <div *ngIf="chessEngine.turn() === 'b' && !chessEngine.isGameOver()" class="text-xs font-medium text-amber-400 px-2.5 py-1 bg-amber-500/10 border border-amber-500/20 rounded-lg">
          To Move
        </div>
      </div>
      ...
      <div
        *ngIf="chessEngine.isGameOver() || chessEngine.isCheck()"
        ...
        class="p-3 rounded-xl border flex items-center justify-center gap-2.5 text-center font-semibold text-sm animate-pulse"
      >
        ...
        <svg *ngIf="chessEngine.isCheck() && !chessEngine.isGameOver()" xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-red-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
      ...
      <div
        [class]="chessEngine.turn() === 'w' ? 'bg-slate-900 border-amber-500/50 ring-2 ring-amber-500/20' : 'bg-slate-900/60 border-slate-800'"
        class="flex items-center justify-between p-3 rounded-2xl border transition-all duration-200"
      >
        <div class="flex items-center gap-3">
          ...
          <div>
            <div class="flex items-center gap-2">
              <span class="font-semibold text-sm text-slate-200">White Player</span>
        ...
        <div *ngIf="chessEngine.turn() === 'w' && !chessEngine.isGameOver()" class="text-xs font-medium text-amber-400 px-2.5 py-1 bg-amber-500/10 border border-amber-500/20 rounded-lg">
          To Move
        </div>
      </div>
    </div>
...
export class GameStatusComponent {
  ...
  public getStatusBannerClass(): string {
    if (this.chessEngine.isCheckmate()) {
      return 'bg-emerald-950/80 border-emerald-500 text-emerald-300 shadow-lg shadow-emerald-950/50';
    }
    ...
    if (this.chessEngine.isCheck()) {
      return 'bg-red-950/80 border-red-500 text-red-300 shadow-lg shadow-red-950/50';
    }
